...
scrollbar = Scrollbar(gui, command=canvas.yview)
...
#Title for program
programLabel = Label(canvas, text='FARO Scene Automation Script')
programLabel.configure(bg='#302c34', fg='#fafafa', font=('Aerial', 20, 'bold'))
programLabel.pack()
programLabel.place(y=240, x=200)

#Start Script Button
addFileButton = tk.Button(canvas, text='Start', padx=10, pady=5, fg='black', bg='#889099', command=confirmList, font=('Aerial', 14, 'bold'))
addFileButton.pack()
...
for job in job_list:
    ...
    var.append(str(i))
    var[i] = tk.IntVar()
    box = tk.Checkbutton(canvas, text=job[0] + ' ' + job[2] + ' ' + job[3], variable=var[i], onvalue=1, offvalue=0)
